...
import { FETCH_CURRENT_COUNTRY, SIGN_UP, CONFIRM_SIGN_UP } from 'circles-mobile/lib/constants/OnboardingConstants'
    ...
    if (err.code === 'UsernameExistsException') {
      try {
        ...
        if (response.constructor.name === 'CognitoUser') {
          ...
          // dispatch the navigation action to the home screeen
...
export function * confirmSignup (action) {
  ...
  try {
    ...
    if (response === 'SUCCESS') {
      ...
      // let response = yield Auth.signIn(action.confirmLoginDetails.phone, action.confirmLoginDetails.password)
      // if (response.constructor.name === 'CognitoUser') {
      //   yield put(addData({isSignedIn: true}))
      //   // dispatch the navigation action to the home screeen
      //   NavigationService.resetFromTo('Main', 'Tabs')
