...
import { FETCH_CURRENT_COUNTRY, SIGN_UP, CONFIRM_SIGN_UP, SAVE_PROFILE_IMAGE } from 'circles-mobile/lib/constants/OnboardingConstants'
// import {
...
export function * signUp (action) {
  try {
    let response = yield Auth.signUp({
      username: action.loginDetails.phone,
      password: action.loginDetails.password,
      attributes: {
        email: action.loginDetails.email,
        name: action.loginDetails.name,
        phone_number: action.loginDetails.phone
      }
    })
    console.log('signUp response', response)
    yield put(addData({name: action.loginDetails.name, phone: action.loginDetails.phone, email: action.loginDetails.email}))
  } catch (err) {
    console.log(err)
    if (err.code === 'UsernameExistsException') {
      try {
        let response = yield Auth.signIn(action.loginDetails.phone, action.loginDetails.password)
        if (response.constructor.name === 'CognitoUser') {
          yield put(addData({name: action.loginDetails.name, phone: action.loginDetails.phone, email: action.loginDetails.email, isSignedIn: true}))
...
export function * confirmSignup (action) {
  ...
  try {
    let response = yield Auth.confirmSignUp(action.confirmLoginDetails.phone, action.confirmLoginDetails.code)
...
export function * saveProfileImage (action) {
  ...
  try {
    ...
    yield put(addData({photoUrl: getResponse}))
  } catch (err) {
