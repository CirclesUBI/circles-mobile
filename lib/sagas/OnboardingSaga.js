...
export function * saveProfileImage () {
  while (true) {
    ...
    try {
      ...
      yield put(addOnboardingData({picture: manipResult}))
    } catch (err) {
...
export function * fetchCurrentCountry () {
  ...
  try {
    ...
    yield put(addOnboardingData({country: responseJson.country}))
    return true
  } catch (err) {
    logger.error(err)
  }
...
export function * getSuggestedContacts () {
  ...
  try {
    ...
    logger.debug('getSuggestedContacts POST response', response)
    yield put(addOnboardingData({suggestedContactIds: response}))
  } catch (err) {
    logger.error(err)
  } finally {
    yield put(hideLoadingSpinner())
