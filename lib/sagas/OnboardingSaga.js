...
import { takeEvery, call, put, select, all, actionChannel } from 'redux-saga/effects'
...
import { signUp, confirmSignUp } from 'circles-mobile/lib/actions/AuthActions'
import { FETCH_CURRENT_COUNTRY, SAVE_PROFILE_IMAGE, INIT_SIGN_UP, INIT_CONFIRM_SIGN_UP, SIGN_UP_SUCCESS, SIGN_UP_FAIL } from 'circles-mobile/lib/constants/OnboardingConstants'
import { SIGN_UP, CONFIRM_SIGN_UP } from 'circles-mobile/lib/constants/AuthConstants'
...
export function * initSignUp (action) {
  console.log('inits', action.payload)
  try {
    const { response, error } = yield call(signUp, action.signUpDetails)
    console.log('initSignUp response', response, error)
...
export function * initConfirmSignup (action) {
  console.log('initConfirmSignup', action)
  try {
    let response = yield put({ type: 'CONFIRM_SIGN_UP', action })
...
export function * signUpFail (err) {
  ...
  try {
    if (err.code === 'CodeMismatchException') yield put(verifyUser('incorrect'))
      ...
      try {
        // let response = yield put({ type: 'SIGN_UP', action })
        let response = yield Auth.signIn(action.signUpDetails.phone, action.signUpDetails.password)
        if (response.constructor.name === 'CognitoUser') {
          yield put(addData({name: action.signUpDetails.name, phone: action.signUpDetails.phone, email: action.signUpDetails.email, isSignedIn: true}))
...
export function * saveProfileImage (action) {
  console.log('saveProfileImage', action)
...
function * onboardingSaga () {
  yield all([
    ...
    takeEvery(SAVE_PROFILE_IMAGE, saveProfileImage),
    takeEvery(INIT_SIGN_UP, initSignUp),
    takeEvery(INIT_CONFIRM_SIGN_UP, initConfirmSignup),
    takeEvery(FETCH_CURRENT_COUNTRY, fetchCurrentCountry),
    takeEvery(SIGN_UP_SUCCESS, signUpSuccess),
    takeEvery(SIGN_UP_FAIL, signUpFail)
  ])
}
