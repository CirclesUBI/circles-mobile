...
export function * signUp (loginDetails) {
  try {
    ...
    yield put(addData({name: loginDetails.name, phone: loginDetails.phone}))
  } catch (err) {
    if (err.code === 'UsernameExistsException') {
      let response = yield Auth.signIn(loginDetails.phone, loginDetails.password)
      if (response.constructor.name === 'CognitoUser') {
        yield put(addData({phone: loginDetails.phone, isSignedIn: true}))
