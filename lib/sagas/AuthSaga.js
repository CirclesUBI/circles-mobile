  ...
  while (true) {
    ...
    try {
      ...
      if (response.constructor.name === 'CognitoUser') {
        console.log('callign initGetOrAddUser')
...
export function * initGetOrAddUser () {
  try {
    ...
    console.log('getting API response')
      // const encodedValue = encodeURIComponent(JSON.stringify(cognitoUser.attributes))
      let response = yield fetch(`http://localhost:8080/v1.1.2/users`, {
        method: 'GET',
        ...
        // body: JSON.stringify(cognitoUser.attributes),
        response: true
      })
    ...
    if (!response.data) {
      ...
      response = yield fetch('http://localhost:8080/v1.1.2/users', {
        method: 'POST',
        mode: "cors", // no-cors, cors, *same-origin
        cache: "no-cache", // *default, no-cache, reload, force-cache, only-if-cached
        credentials: "same-origin", // include, *same-origin, omit
