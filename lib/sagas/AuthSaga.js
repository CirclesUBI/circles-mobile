...
import { addRecoveryData, setUsernameExists } from 'circles-mobile/lib/actions/RecoveryActions'
...
export function * confirmForgotPassword () {
  while (true) { 
    ...
    try {
      ...
      logger.info('confirmForgotPassword wallet', wallet)  
      if (!wallet)
...
export function * checkUsernameExists () {
  logger.info('checkUsernameExists')
  while (true) {
    const action = yield take(CHECK_USERNAME_EXISTS)
    const { username, type } = action.data
    let exists = false
    try {
      yield put(showLoadingSpinner())
      const code = '000000'
      ...
      yield put(setUsernameExists(exists))
      yield put(hideLoadingSpinner())
    }
  }
}
...
function * authSaga () {
  ...
  yield fork(checkUsernameExists)
}
