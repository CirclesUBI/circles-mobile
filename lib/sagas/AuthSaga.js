    ...
    try {
      ...
      if (response) {
        ...
        if (finishedOnboarding) {
          ...
          logger.info('addDbUserData response: ' + response)
    ...
    try {
      ...
      let signUpParams = {
        ...
        attributes: {
          ...
          phone_number: phone,
          picture: picture,
          'custom:device_id': DeviceInfo.getUniqueID() // for push notifications
        }
      }
      ...
      logger.info('initSignUp response', response)
    } catch (err) {
...
export function * initAddUser (password) {
  try {
    ...
    // yield call(createCirclesToken)
    const tokenAddress = '0x01' // yield select((state) => state.user.token_address)
    const cognitoUser = yield Auth.currentAuthenticatedUser()
    ...
    logger.info('initAddUser Auth.signIn response', signInResponse)
    NavigationService.navigate('SeedPhrase')
  } catch (err) {
    logger.error(err)
  } finally {
...
export function * createCirclesToken () {
  logger.info('createCirclesToken')    
  const cognitoUser = yield Auth.currentAuthenticatedUser()  
  logger.info('cognitoUser: ' + cognitoUser)
  ...
  logger.info('senderKeyPair: ' + senderKeyPair)
  ...
  try {    
    ...
    logger.info('create POST response', response)
  } catch (error) {
    logger.error(error)
  }
}
...
export function * initRecoverAccount () {  
  while (true) {    
    ...
    console.log('initRecoverAccount')
    try {
      yield put(showLoadingSpinner())
      let w = yield SecureStore.getItemAsync('wallet')
      logger.info('wallet: '+ w)
      let wallet = JSON.parse(w)
      ...
      logger.info('initRecoverAccount POST response', response)
      yield put(addDbUserData(response))
