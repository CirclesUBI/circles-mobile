...
export function * initAddUser (password) {
  try {
    ...
    console.log('initAddUser POST response', response)
    ...
    console.log('initAddUser Auth.signIn response', signInResponse)
    NavigationService.navigate('SeedPhrase')
  } catch (err) {
    logger.error(err)
  } finally {
...
export function * createCirclesToken () {
  ...
  logger.info('to')
  const data = HubContract.interface.functions.relayerSignup.encode([senderKeyPair.address, cognitoUser.username])
  logger.info('data')
  ...
  try {
    ...
    logger.info('createCirclesToken post response: ' + response)
    yield put(userTokenCreated(response.data))
    logger.info('userTokenCreated')
  } catch (error) {
    logger.error(error)
  }
}
