...
>>>>>>> Update location of contract authorization
        yield put({ type: 'SIGNIN_REQUEST', data: action.data })
...
export function * initGetOrAddUser () {
  try {
    ...
    const apiName = 'user'
    const path = '/' + cognitoUser.username
    ...
    logger.debug('initGetOrAddUser GET response', response)
    if (!response.data) {
      response = yield API.post(apiName, path, myInit)
      logger.debug('initGetOrAddUser POST response', response)
    }
...
export function * createCirclesToken () {
  ...
  if (!err) {
    ...
    logger.error(err)
=======
export function * createCirclesPerson (name) {
  let randomWallet = ethers.Wallet.createRandom()
  let err = yield SecureStore.setItemAsync('wallet', JSON.stringify(randomWallet))
  if (!err) {
    ...
    let transaction = {
      ...
      gasPrice: yield customHttpProvider.getGasPrice(),
      to: HubContract.address,
      data: HubContract.interface.functions.signup.encode([name])
    }
    let tx = yield connectedWallet.sign(transaction)
    this.props.createCirclesPerson(randomWallet.address, tx)
