...
import { Response } from 'aws-sdk/global';
...
export function * initAddUser () {
  try {
    ...
    const myInit = { // OPTIONAL
      // body: Object.assign({}, cognitoUser.attributes, { address: wallet.address }),
    ...
    yield call(createCirclesToken(cognitoUser.username))
...
export function * createCirclesToken (username) {
  console.log('calling createCirclesToken')
  ...
  if (wallet) {
    ...
    let data = HubContract.interface.functions.signup.encode([senderKeyPair.address, 'test'])
    // Tight packing, as Solidity sha3 does
    let hashInput = '0x1900' + RelayContract.address.slice(2) +
      '0x0000000000000000000000000000000000000000'.slice(2) + leftPad(nonce.slice(2), '64', '0') + to.slice(2) + data.slice(2)
    var hash = ethers.utils.keccak256(hashInput)
    var sig = yield wallet.signMessage(hash)
    let expandedSig = ethers.utils.splitSignature(sig)
    let abi = RelayContract.interface.abi
    let functionName = 'relayMetaTx'
    let args = [ expandedSig.v,
      expandedSig.r,
      expandedSig.s,
      to,
      data,
    ...
    txObject.gasPrice = add0x(yield customHttpProvider.getGasPrice())
    txObject.gasLimit = add0x(ethers.utils.hexlify(1600000))
