...
export function * initAddUser () {
  try {
    ...
    const token = cognitoUser.signInUserSession.accessToken.jwtToken
    const myInit = { // OPTIONAL
      body: Object.assign({}, cognitoUser.attributes, {wallet_address: wallet.address, token_address: tokenAddress}), //{ wallet_address: wallet.address, token_address: tokenAddress }),
      headers: {
        accesstoken: token
      }
    }
