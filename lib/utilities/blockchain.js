...
import MetaTxGenerator from 'metatx-client'
...
export function generateKeyPair (privKey) {
  try {
    let wallet = new ethers.Wallet(privKey, this.provider)
    return {
...
let metaTxGenerator = new MetaTxGenerator(customHttpProvider, RelayAddress, RelayABI)
