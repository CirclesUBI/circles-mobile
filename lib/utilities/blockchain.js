...
import MetaTx from 'metatx-client'

const logger = new ConsoleLogger('Blockchain')

export const customHttpProvider = new ethers.providers.JsonRpcProvider(RPC_URL)

logger.debug('RPC_URL', RPC_URL)
customHttpProvider.getNetwork().then( x => logger.debug(x))
...
export const metaTxGenerator = new MetaTx(customHttpProvider, RelayAddress, RelayABI)
