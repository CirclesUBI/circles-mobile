...
// Styles
import { background1, fonts, primary, textColor4, textColor1, textColor2 } from 'circles-mobile/lib/styles'
...
export class OnboardingVerifyPhone extends React.Component {
  ...
  render () {
    const formattedPhone = internationalFormat(this.props.userData.phone, this.props.userData.country)
    return (
      <View style={{flex: 1, backgroundColor: background1}}>
        <NavBar navFunction={() => this.props.navigation.goBack()} title='Activation Code' />
        <View style={{flex: 0.9}}>
          <View>
            <Text style={{textAlign: 'center', color: textColor1, marginTop: 15}}>
          ...
          >
            <Text infoButtonLabel style={{color: textColor2}}>Call Me</Text>
          </TouchableOpacity>
          <TextInput
            style={[
              // styles.pinInput,
            ...
            autofocus
            returnKeyType='go'
            onSubmitEditing={() => this.state.verificationCode && this.state.verificationCode.length === 6 && this.onProcess()}
          />
        </View>
        {
          (this.state && this.state.verificationCode) && <View style={{flex: 0.1, backgroundColor: 'white'}}>
...
export default connect(mapStateToProps, mapDispatchToProps)(OnboardingVerifyPhone)
