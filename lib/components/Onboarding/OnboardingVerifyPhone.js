...
export class OnboardingVerifyPhone extends React.Component {
      ...
      .catch(err => {
        if (err.code === 'UsernameExistsException') {
          AuthManager.signIn(username, password)
            .then(data => {
              console.log('user known, signing in', data)
              this.props.navigation.navigate('HomeScreen')
            })
            .catch(err => {
              if (err.code === 'UserNotConfirmedException') {
                console.log('UserNotConfirmedException', err) // return AuthManager.resendSignUp(username)
  ...
  // async saveUser (USER_KEY, username) {
  //   return AsyncStorage.setItem(USER_KEY, username)
  // }
  ...
  onProcess () {
    ...
    AuthManager.confirmSignup('edzillion', this.state.verificationCode)
      .then(res => {
        if (res === 'SUCCESS') this.setState({ buttonMessage: 'VERIFIED', verified: true, incorrect: false })
