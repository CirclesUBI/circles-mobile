...
export class OnboardingPhone extends React.Component {
  constructor (props) {
    ...
    this.onProcessPhone = this.onProcessPhone.bind(this)
  ...
  onProcessPhone () {
    ...
    if ((this.state.phone && this.state.phone.length < 10) || !PhoneNumberUtil.isViablePhoneNumber(this.state.phone)) {
      this.setState({ phoneValid: false, phone: this.state.phone })
      Toast.fail('Please enter a valid phone number', 3)
    } else {
      this.setState({ phoneValid: true, phone: this.state.phone })
      let phoneNumObj = this.phoneUtil.parseAndKeepRawInput(this.state.phone || '', country)
