...
export class OnboardingAvatar extends React.Component {
  ...
  async openCamera () {
    ...
    // const { camRollPermission } = await Permissions.askAsync(Permissions.CAMERA_ROLL)
    // this.setState({ hasCameraPermission: status === 'granted', hasCameraRollPermission: status === 'granted })
    // console.log(camPermission, camRollPermission)
    if (camPermission === 'granted' && camRollPermission === 'granted') {
  ...
  render () {
    return (
      <View style={{ flex: 1, backgroundColor: background1 }}>
        ...
        {this.state.name && this.props.userData.picture ? (
