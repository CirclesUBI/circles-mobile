...
import { background1, textColor1, fonts, primary } from 'circles-mobile/lib/styles'
  ...
  render () {
    return (
      <View style={{flex: 1, backgroundColor: background1}}>
        <NavBar navFunction={() => this.props.navigation.goBack()} title='Enter Your Information' />
        <View style={{flex: 0.9}}>
          <TouchableHighlight
            ...
            <View>
              { this.props.userData.avatar
                ...
                : <View style={styles.emptyAvatar}>
                  <Text infoButtonLabel style={{color: textColor1}}>Upload Photo</Text>
                </View>}
              {!this.props.userData.avatar && <Text style={{color: textColor1, textAlign: 'center', marginTop: 15}}>Tap to upload</Text>}
            </View>
        ...
        {
          (this.state && this.state.name) && <View style={{flex: 0.1, backgroundColor: 'white'}}>
            <TouchableHighlight style={[styles.loginButton, {backgroundColor: primary}]} onPress={this.onProcess}>
              <View>
...
export default connect(mapStateToProps, mapDispatchToProps)(OnboardingAvatar)
