      ...
      <View style={{flex: 1, alignItems: 'center'}}>
        ...
        >
          ...
          <View style={{alignItems: 'center', marginTop: calculateHeightRatio(180)}}>
            <Button onClick={() => {
              this.props.navigation.navigate(this.props.wallets[this.props.user.selectedWallet] && this.props.wallets[this.props.user.selectedWallet].primary ? 'HomeScreen' : 'OrgHome')
            }} style={[styles.loginButton, {backgroundColor: '#9353C8', marginBottom: calculateHeightRatio(20)}]}>
              <Text style={styles.loginText}>{this.translate('CONNECT WITH UPORT')}</Text>
            </Button>
            <Button onClick={() => {
              this.props.navigation.navigate(this.props.wallets[this.props.user.selectedWallet] && this.props.wallets[this.props.user.selectedWallet].primary ? 'HomeScreen' : 'OrgHome')
            }} style={[styles.loginButton, {backgroundColor: '#536EC8', marginBottom: calculateHeightRatio(23)}]}>
              <Text style={styles.loginText}>{this.translate('CONNECT WITH FACEBOOK')}</Text>
            </Button>
            <Button onClick={() => {
              this.props.navigation.navigate(this.props.wallets[this.props.user.selectedWallet] && this.props.wallets[this.props.user.selectedWallet].primary ? 'HomeScreen' : 'OrgHome')
            }} style={[styles.loginButton, {backgroundColor: '#F5F5F5', marginBottom: calculateHeightRatio(33)}]}>
              <Text style={[styles.loginText, {color: '#353535'}]}>{this.translate('CONNECT WITH GOOGLE')}</Text>
            </Button>
          </View>
        </LinearGradient>
      </View>
