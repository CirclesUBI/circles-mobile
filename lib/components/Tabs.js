...
import { selectedTransactionWallet } from 'circles-mobiles/lib/actions/UserActions'
...
import { mediumLightBackground, textColor4 } from 'circles-mobile/lib/styles/styles'
...
const AccountsListViewComponent = (props) => {
  ...
  let wallets = walletNames.map((wallet, i) => (
    <TouchableWithoutFeedback key={i} onPress={() => {
      ...
      <View style={{height: calculateHeightRatio(45), alignItems: 'center', justifyContent: 'center'}}>
        <Text style={{fontFamily: 'now-alt-regular', color: textColor4, fontSize: 14}}>
          {walletNames[i]}
        </Text>
      </View>
  ...
  return (
    <View style={{backgroundColor: mediumLightBackground, marginLeft: -5, marginRight: -5, marginTop: -5, height: calculateHeightRatio(150)}}>
      <View style={{backgroundColor: mediumLightBackground, height: 60, justifyContent: 'center', borderBottomWidth: 1, borderColor: 'rgba(58,59,78, 49)'}}>
        <Text style={{color: 'white', fontSize: 18, fontFamily: 'now-alt-bold', textAlign: 'center'}}>
          Select Wallet
        </Text>
      </View>
      <ScrollView>
...
const AccountsListView = connect(mapStateToProps0, mapDispatchToProps0)(AccountsListViewComponent)
  ...
  render () {
    ...
    return (
      <View style={{height: 60, flexDirection: 'row', justifyContent: 'space-around'}}>
        <LinearGradient
          colors={[mediumLightBackground, '#151520']}
