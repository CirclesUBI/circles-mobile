...
class OrgHomeScreen extends React.Component {
  ...
  render () {
    ...
    let wallets = walletNames.map((wallet, i) => {
      return walletNames[i] !== this.props.user.selectedOrgWallet
      ? <MenuOption onSelect={() => this.walletSelectionHandler(walletNames[i])} key={i}>
            ...
            <Menu style={{marginRight: 18}}>
              <MenuTrigger>
                <View style={{flexDirection: 'row', alignItems: 'center'}}>
                  <Text style={{color: 'white', fontSize: 20, fontFamily: 'now-alt-bold', marginRight: 13}}>{this.props.user.selectedOrgWallet}</Text>
              ...
              <MenuOptions customStyles={{optionsContainer: {marginTop: 30}}}>
                <MenuOption onSelect={() => this.props.navigation.navigate('HomeScreen')}>
                  <Text style={{fontSize: 20, fontFamily: 'now-alt-regular'}}>{this.props.user.name}</Text>
                </MenuOption>
                {wallets}
          ...
          <View style={{backgroundColor: darkBackground, flexDirection: 'row', justifyContent: 'space-between', height: calculateHeightRatio(76), alignItems: 'center'}}>
            <View style={{flexDirection: 'column', marginLeft: 20}}>
              <Text style={{color: 'white', fontSize: 26, fontFamily: 'now-alt-bold'}}>
                {numberWithCommas(this.props.wallets[this.props.user.selectedOrgWallet].balance)}
              </Text>
